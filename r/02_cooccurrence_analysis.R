...
option_list <- list(
  ...
  make_option("--mod_type", type = "character", default = "logistf",
              help = glue("Type of model to use in cooccurrence analysis - ",
                          "logistf for SPAtest [default = %default]"))
)
...
ukb_tr_pims <- lapply(seq_along(time_thresholds),
                      \(x) {glue("data/private/ukb/{opt$ukb_version}/X","{gsub('X', '', opt$outcome)}/",
                                 ...
                                 "{time_thresholds[i]}_{opt$ukb_version}.fst") |>
                          read_fst(as.data.table = TRUE)})
...
## mgi
mgi_results <- lapply(seq_along(time_thresholds),
       \(x){
         output_cooccurrence_results(
...
## ukb
ukb_results <- lapply(seq_along(time_thresholds),
                      \(x) {
                        output_cooccurrence_results(
                          pim_data     = ukb_tr_pims[[glue("t{time_thresholds[x]}_threshold")]],
                          t_thresh     = time_thresholds[x],
