...
message("using {format(length(keep_these_vars) - 1, big.mark = ',')} predictors (only 'leaf' phecodes) available in both cohorts...")
d <- d[, ..keep_these_vars]
u <- u[, ..keep_these_vars]
...
message("Trying {opt$n_mtry} values of mtry, {opt$n_node_size} of node size, {opt$n_samp_frac} values of sample fraction ({nrow(hyper_grid)} combinations)")

message("performing grid search...")
pb <- txtProgressBar(max = nrow(hyper_grid), width = 50, style = 3)
...
messasge("fitting random forest with optimal hyperparameters based on grid search...")
pb <- txtProgressBar(max = length(oob_rmse), width = 50, style = 3)
