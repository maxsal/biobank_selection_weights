...
prepped_nhis <- fread("https://raw.githubusercontent.com/maxsal/public_data/main/prepped_nhis.csv")
...
(nhis_demo_summary <- weighted_summary_wrapper(
    prepped_nhis,
    dsn = dsn,
    vars = c(
...
fwrite(
    x    = nhis_demo_summary,
    file = glue("data/public/nhis_weighted_demo_summary.txt"),
    sep  = "\t"
)
