...
d <- read_fst(glue("data/private/mgi/{opt$mgi_version}/X{gsub('X', '', opt$outcome)}/time_restricted_phenomes/mgi_X{gsub('X', '', opt$outcome)}_t{opt$time_threshold}_{opt$mgi_version}.fst"),
              as.data.table = TRUE)
d_weights <- read_fst(glue("data/private/mgi/{opt$mgi_version}/weights_{opt$mgi_version}_comb.fst"),
              as.data.table = TRUE)
d <- merge.data.table(d, d_weights, by = "id", all.x = TRUE)
d <- d[!is.na(cancer_ipw), ]
d_ids <- d[, .(id, case)]

sample_index <- sample(x = seq_len(nrow(d)), size = 10000, replace = TRUE, prob = 1/d[, cancer_ipw])
d_sample <- d[sample_index, ]
drop <- unique(c(names(d_sample)[names(d_sample) %in% names(d_weights)]))
keep <- c("id", "case", names(d_sample)[!(names(d_sample) %in% drop)])
d_sample <- d_sample[, ..keep]
