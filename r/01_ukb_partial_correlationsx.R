...
pim0 <- read_qs(glue("data/private/ukb/{opt$ukb_version}/UKB_PIM0X_{opt$ukb_version}.qs"))
...
## demographics data
demo <- read_qs(glue("data/private/ukb/{opt$ukb_version}/datax_{opt$ukb_version}_comb.qs"))[
...
output_file <- glue("data/private/ukb/{opt$ukb_version}/",
                    "ukb_phenome_partial_correlationsx_",
...
woutput_file <- glue(
  ...
  "ukb_phenome_weighted_partial_correlationsx_",
  "{ifelse(opt$use_geno == TRUE, 'w_geno_pcs_', '')}",
  "{opt$ukb_version}.qs"
)
message("saving results to: {.path {output_file}}")
