plotlyman_log10p <- function(data, phe_data, thresh = NULL, outcome) {
  ...
  symbols = c("Up" = "triangle-up", "Down" = "triangle-down")
  ...
  plot_ly(data = data[!is.na(beta)]) |>
    ...
    add_trace(
      x = ~slide_num, y = ~log10p,
      type = "scatter", mode = "markers",
      color = ~group, colors = col_vals,
      symbol = ~shape, symbols = symbols,
      marker = list(size = 10),
