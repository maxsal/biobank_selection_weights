...
getTopEffects <- function(prob, pred, cov, dat) {
  add_cov <- ifelse(length(cov) == 0, "", paste0(" + ", paste0(cov, collapse = " + ")))
  riskBin               <- glue::glue("Top_{prob}")
  dat[[riskBin]] <- ifelse(dat[[pred]] >= quantile(dat[case == 0, ][[pred]], probs = 1 - prob), 1, 0)
  tmp_vars <- c("case", riskBin, cov)
  fitAsso2 <- logistf(as.formula(paste0("case ~ ", riskBin, add_cov)), data = na.omit(dat[, ..tmp_vars]), control = logistf.control(maxit = 1000))
...
getValues <- function(fitAsso, predictor) {
  ...
  LOG10P      <- -pchisq((fitAsso$coefficient[predictor]^2/vars[predictor]), 1, lower.tail = F,log=T)/log(10)
  P           <- log10toP(LOG10P)
  ...
  names(out) <- paste0(predictor,"_",c("BETA","SEBETA","P","OR","CI1","CI2","LOG10P"))
  return(out)
}
...
evaluate_phers <- function(
  ...
  add_cov <- ifelse(length(covariates) == 0, "", paste0(" + ", paste0(covariates, collapse = " + ")))
  
  # m1: adjusted GLM model
  message("m1: adjusted GLM model")
  m1   <- stats::glm(paste0("case ~ ", predictor, add_cov), family = binomial(), data = eval_pim)
  vars <- diag(vcov(m1))
  
  if (coef(summary(m1))[predictor, 4] == 0) {
    out$p_value <- log10toP(-pchisq((m1$coefficient[predictor]^2 / vars[predictor]), 1, lower.tail = F, log = T) / log(10))
  } else {
  ...
  out$or_ci  <- glue::glue("{round(exp(m1_confint[predictor, 1]), 5)}, {round(exp(m1_confint[predictor, 2]), 5)}")
  out$log10p <- -pchisq((m1$coefficient[[predictor]]^2/vars[[predictor]]), 1, lower.tail = F,log=T)/log(10)
  
  # auc
  message("m1 AUC")
  m1_auc     <- pROC::roc(response = eval_pim[["case"]], predictor = eval_pim[[predictor]], family = binomial(), ci = TRUE)
  out$auc    <- m1_auc$auc
  ...
  if (calc_r2 == TRUE) {
    ...
    out$r2_nagelkerke    <- signif(cs_m2 / (1- exp((2/N_m2) * n_m2)), digits = 6)
  }
  
  m2_hl           <- ResourceSelection::hoslem.test(m2$y, fitted(m2), g = 10)
  out$hl_chisq    <- m2_hl$statistic[["X-squared"]]
  out$hl_p        <- m2_hl$p.value
  out$brier_score <- DescTools::BrierScore(m2)
  ...
  for(g in 1:G){
    ...
    fit.train   <- logistf(as.formula(formula.fit), data=traindata, control = logistf.control(maxit = 1000))
    ...
    formula.predict            <- glue("logistf(case ~ fittedPredictions, family = binomial(logit), data = testdata, ci = TRUE, control = logistf.control(maxit = 1000))")
    predict.test               <- eval(parse(text=formula.predict))
    linearpredictors[[g]]      <- data.table("id" = testdata$id,'linearpredictor' = predict.test$linear.predictors)
  }
  ...
  if (pctile_or == TRUE) {
    ...
    for(prob in c(0.01, 0.02, 0.05, 0.1, 0.25)){
      ...
      out <- c(out, getTopEffects(prob, pred = predictor, cov = covariates, dat = eval_pim))
    }
    
    # determine percentile with at least 80% power (or the most powered percentiles)
    ptest    <- sapply(seq(0.005, 0.5, by = 0.005),
                       \(x) {getTopPower(prob = x, dat = eval_pim, pred = predictor)})
    ...
    prs.p80        <- getTopEffects(p80, pred = predictor, cov = covariates, dat = eval_pim)
...
calculate_phers <- function(
  ...
  pb <- progress::progress_bar$new(total = length(phers_hits[, phecode]),
